...
마감 CTA 문구를 매일 토씨 하나 안 틀리고 완전히 동일하게 반복 게시했더니, 이런
패턴(똑같은 홍보문구 반복)이 스팸/사기 계정의 전형적 시그니처로 잡힐 수 있다는
지적을 받아 card_templates._rot와 같은 방식(날짜 시드)으로 로테이션을 준다.
안내문구(면책조항)는 법적 성격이라 일부러 고정해 두고 CTA·링크 문구만 돌린다."""
from __future__ import annotations

from card_templates import _rot
...
_CTA_LINES = [
    ...
    "재무제표부터 회계감사의견까지, 전 종목 무료 스크리닝 — 머니체크업에서 확인하세요.",
...
def _closing_lines(today_str, tag):
    return [
        ...
        _rot(f"{today_str}|{tag}|link", _LINK_LINES),
        "",
...
def build_caption(data, headline_lines, subtitle, today_str):
...
def build_weekly_caption(data, headline_lines, subtitle, today_str):
